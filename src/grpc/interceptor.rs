...
pub fn jwt_gateway_interceptor(mut req: Request<()>) -> Result<Request<()>, Status> {
...
/// Auth interceptor used by GRPC services. Verifies JWT token sent
/// in GRPC metadata under "authorization" key.
pub fn jwt_yubibridge_interceptor(mut req: Request<()>) -> Result<Request<()>, Status> {
    let token = match req.metadata().get("authorization") {
        Some(token) => token
            .to_str()
            .map_err(|_| Status::unauthenticated("Invalid token"))?,
    ...
    if let Ok(claims) = Claims::from_jwt(ClaimsType::YubiBridge, token) {
        // FIXME: can we push whole Claims object into metadata?
        req.metadata_mut().insert(
            "username",
            claims
