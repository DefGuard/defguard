...
async fn main() -> Result<(), anyhow::Error> {
    ...
    tokio::select! {
        ...
        res = run_web_server(worker_state, gateway_state, webhook_tx, webhook_rx, wireguard_tx.clone(), mail_tx.clone(), pool.clone(), failed_logins) => error!("Web server returned early: {res:?}"),
