...
pub async fn get_network_device(
    ...
    if let Some(device) = device {
        if device.device_type == DeviceType::Network {
            let mut transaction = appstate.pool.begin().await?;
            let network_device_info = to_network_device_info(device, &mut transaction).await?;
            transaction.commit().await?;
            return Ok(ApiResponse {
...
async fn to_network_device_info(
    device: Device<Id>,
    transaction: &mut PgConnection,
) -> Result<NetworkDeviceInfo, WebError> {
...
pub(crate) async fn add_network_device(
    ...
    if !user.is_active && !session.is_admin {
        info!(
            "User {} tried to manage network devices despite not being an active admin.",
            user.username
        );
    ...
    info!(
        "User {} added a network device {device_name}.",
        user.username
    );
