...
use crate::{
    ...
    db::{
        models::{device::DeviceInfo, enrollment::{Token, PASSWORD_RESET_TOKEN_TYPE}}, AppEvent, GatewayEvent, MFAMethod, OAuth2AuthorizedApp, Settings, User, UserDetails, UserInfo, Wallet, WebAuthn, WireguardNetwork
    },
    error::WebError,
...
pub async fn delete_user(
    ...
    if let Some(user) = User::find_by_username(&appstate.pool, &username).await? {
        ...
        debug!("User {} deleted user {username}, purging their network devices across all networks.", session.user.username);
        let devices = user.devices(&appstate.pool).await?;
        let mut transaction = appstate.pool.begin().await?;
        let mut events = Vec::new();
        for device in devices {
            events.push(GatewayEvent::DeviceDeleted(DeviceInfo::from_device(&mut *transaction, device.device).await?));
        }
