...
pub async fn modify_user(
    ...
    let old_rdn = user.ldap_rdn_value().to_string();
    ...
    if !user_info.is_active && status_changing {
        ...
        ldap_delete_user(&old_rdn, &appstate.pool).await;
        ...
        ldap_modify_user(&old_rdn, &user, &appstate.pool).await;
    }
...
pub async fn delete_user(
    ...
    if let Some(user) = User::find_by_username(&appstate.pool, &username).await? {
        ...
        let mut transaction = appstate.pool.begin().await?;
        let user_rdn = user.ldap_rdn_value().to_string();
        ...
        transaction.commit().await?;
        update_counts(&appstate.pool).await?;
        ldap_delete_user(&user_rdn, &appstate.pool).await;
