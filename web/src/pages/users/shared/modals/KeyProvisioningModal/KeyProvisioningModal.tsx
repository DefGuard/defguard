...
export const KeyProvisioningModal = () => {
  ...
  const [jobStatusSubject, setJobStatusSubject] = useState<
  ...
  useQuery(
    [QueryKeys.FETCH_WORKER_JOB_STATUS, jobId],
    () => getJobStatus(jobId),
    {
  ...
  return (
    <Modal
      ...
      className={
        submitted
          ? 'key-provisioning middle submitted'
          : 'key-provisioning middle'
      }
    >
      {!submitted ? (
        <motion.div
