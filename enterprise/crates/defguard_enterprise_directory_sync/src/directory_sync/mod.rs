...
use defguard_common::db::{
    Id,
    models::{Settings, group::Group, settings::OpenIdUsernameHandling, user::User},
};
use defguard_enterprise_db::models::openid_provider::{
    DirectorySyncTarget, DirectorySyncUserBehavior, OpenIdProvider,
};
#[cfg(not(test))]
use defguard_enterprise_license::is_business_license_active;
use defguard_enterprise_ldap::{
    model::ldap_sync_allowed_for_user,
    utils::{
        ldap_add_users_to_groups, ldap_delete_users, ldap_remove_users_from_groups,
...
type DisableUserFn = dyn for<'b> Fn(
        &'b mut User<Id>,
        &'b mut PgConnection,
    ) -> BoxFuture<'b, Result<(), DirectorySyncError>>
    + Send
    + Sync
    + 'static;

type DeleteUserFn = dyn for<'b> Fn(
        User<Id>,
        &'b mut PgConnection,
    ) -> BoxFuture<'b, Result<(), DirectorySyncError>>
    + Send
...
type SyncAllowedDevicesFn = dyn for<'b> Fn(
        &'b User<Id>,
        &'b mut PgConnection,
    ) -> BoxFuture<'b, Result<(), DirectorySyncError>>
    + Send
...
pub struct DirectorySyncContext {
    pub disable_user: Box<DisableUserFn>,
    pub delete_user_and_cleanup_devices: Box<DeleteUserFn>,
    pub sync_allowed_user_devices: Box<SyncAllowedDevicesFn>,
}
...
fn check_username(username: &str) -> Result<(), DirectorySyncError> {
    ...
    if let Some(first_char) = username.chars().next() {
        if !first_char.is_ascii_alphanumeric() {
            return Err(DirectorySyncError::UserCreateError(
...
#[must_use]
fn prune_username(username: &str, handling: OpenIdUsernameHandling) -> String {
...
pub async fn sync_user_groups_if_configured(
    ...
    match DirectorySyncClient::build(pool).await {
        Ok(mut dir_sync) => {
            dir_sync.prepare().await?;
            sync_user_groups(&dir_sync, user, pool, context).await?;
        }
        Err(err) => {
            error!("Failed to build directory sync client: {err}");
        }
            ...
            match admin_behavior {
                ...
                DirectorySyncUserBehavior::Disable => {
                    if user.is_active {
                        ...
                        (context.disable_user)(&mut user, &mut transaction)
                            .await
                            .map_err(|err| {
                ...
                DirectorySyncUserBehavior::Delete => {
                    ...
                    (context.delete_user_and_cleanup_devices)(user, &mut transaction)
                        .await
                        .map_err(|err| {
                            DirectorySyncError::UserUpdateError(format!(
...
async fn sync_inactive_directory_users(
    ...
    for mut user in users_to_disable {
        if user.is_active {
            ...
            (context.disable_user)(&mut user, transaction)
                .await
                .map_err(|err| {
                    DirectorySyncError::UserUpdateError(format!(
                        "Failed to disable user {} during directory synchronization: {err}",
...
pub async fn get_directory_sync_interval(pool: &PgPool) -> u64 {
    ...
    match DirectorySyncClient::build(pool).await {
        Ok(mut dir_sync) => {
            ...
            if matches!(
                ...
                sync_all_users_state(pool, context, &users).await?;
            ...
            if matches!(
                ...
                sync_all_users_groups(&dir_sync, pool, context, users_to_pass.as_deref()).await?;
            }
        }
        Err(err) => {
            error!("Failed to build directory sync client: {err}");
