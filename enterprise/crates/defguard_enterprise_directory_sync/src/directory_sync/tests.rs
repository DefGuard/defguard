...
mod test {
            ...
            disable_user: Box::new(move |user: &mut User<Id>, conn| {
                ...
                Box::pin(async move {
                    ...
                    for device in devices {
                        let info = DeviceInfo::from_device(&mut *conn, device)
                            .await
                            .map_err(|err| DirectorySyncError::UserUpdateError(err.to_string()))?;
                        let _ = disable_tx.send(TestGatewayEvent::DeviceDeleted(info));
                    }
                    Ok(())
                })
            }),
            delete_user_and_cleanup_devices: Box::new(move |user: User<Id>, conn| {
                ...
                Box::pin(async move {
                    ...
                    for device in devices {
                        let info = DeviceInfo::from_device(&mut *conn, device)
                            .await
                            .map_err(|err| DirectorySyncError::UserUpdateError(err.to_string()))?;
                        let _ = delete_tx.send(TestGatewayEvent::DeviceDeleted(info));
                    }
                    user.delete(&mut *conn).await?;
                    Ok(())
                })
            }),
            sync_allowed_user_devices: Box::new(move |user: &User<Id>, conn| {
                ...
                Box::pin(async move {
                    ...
                    for device in devices {
                        let info = DeviceInfo::from_device(&mut *conn, device)
                            .await
                            .map_err(|err| DirectorySyncError::UserUpdateError(err.to_string()))?;
                        let _ = sync_tx.send(TestGatewayEvent::DeviceCreated(info));
                    }
                    Ok(())
                })
            }),
    ...
    async fn test_users_state_keep_both(_: PgPoolOptions, options: PgConnectOptions) {
        ...
        let (wg_tx, mut wg_rx) = broadcast::channel::<TestGatewayEvent>(16);
        make_test_provider(
            &pool,
        ...
        let context = build_test_context(wg_tx.clone());
        sync_all_users_state(&pool, &context, &all_users)
            .await
            .unwrap();
    ...
    async fn test_users_state_delete_users(_: PgPoolOptions, options: PgConnectOptions) {
        ...
        if let Ok(TestGatewayEvent::DeviceDeleted(dev)) = event {
    ...
    async fn test_users_state_delete_both(_: PgPoolOptions, options: PgConnectOptions) {
        ...
        if let Ok(TestGatewayEvent::DeviceDeleted(dev)) = event1 {
            assert!(
        ...
        if let Ok(TestGatewayEvent::DeviceDeleted(dev)) = event2 {
            assert!(
                dev.device.user_id == user1.id
                    || dev.device.user_id == user2.id
                    || dev.device.user_id == user3.id
    ...
    async fn test_users_state_disable_admins(_: PgPoolOptions, options: PgConnectOptions) {
        ...
        let config = DefGuardConfig::new_test_config();
        let _ = SERVER_CONFIG.set(config.clone());
        let (wg_tx, mut wg_rx) = broadcast::channel::<TestGatewayEvent>(16); // Added mut wg_rx
        make_test_provider(
            &pool,
            DirectorySyncUserBehavior::Keep,
            DirectorySyncUserBehavior::Disable,
    ...
    async fn test_users_groups(_: PgPoolOptions, options: PgConnectOptions) {
        ...
        let config = DefGuardConfig::new_test_config();
        let _ = SERVER_CONFIG.set(config.clone());
        let (wg_tx, _) = broadcast::channel::<TestGatewayEvent>(16);
        make_test_provider(
            &pool,
            DirectorySyncUserBehavior::Delete,
            DirectorySyncUserBehavior::Delete,
        ...
        let all_users = client.get_all_users().await.unwrap();
        let context = build_test_context(wg_tx.clone());
        sync_all_users_groups(&client, &pool, &context, Some(&all_users))
            .await
            .unwrap();
    ...
    async fn test_sync_user_groups(_: PgPoolOptions, options: PgConnectOptions) {
        ...
        let context = build_test_context(wg_tx.clone());
        sync_user_groups_if_configured(&user, &pool, &context)
            .await
            .unwrap();
        let user_groups = user.member_of(&pool).await.unwrap();
    ...
    async fn test_sync_target_users(_: PgPoolOptions, options: PgConnectOptions) {
        ...
        assert_eq!(user_groups.len(), 0);
        let context = build_test_context(wg_tx.clone());
        do_directory_sync(&pool, &context).await.unwrap();
        let user_groups = user.member_of(&pool).await.unwrap();
        assert_eq!(user_groups.len(), 0);
    }
    ...
    async fn test_sync_target_all(_: PgPoolOptions, options: PgConnectOptions) {
        ...
        let mut transaction = pool.begin().await.unwrap();
        let context = build_test_context(wg_tx.clone());
        (context.sync_allowed_user_devices)(&user, &mut transaction)
            .await
            .unwrap();
        transaction.commit().await.unwrap();
        let event = wg_rx.try_recv();
        ...
        let event = wg_rx.try_recv();
        if let Ok(TestGatewayEvent::DeviceCreated(dev)) = event {
