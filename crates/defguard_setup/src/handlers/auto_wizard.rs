...
use defguard_common::db::models::wizard::{ActiveWizard, Wizard};
use defguard_core::{
...
pub(crate) async fn advance_auto_wizard_to_step(
	...
	let auto_state = wizard.auto_adoption_state.get_or_insert_with(Default::default);
...
pub async fn get_auto_adoption_result(Extension(pool): Extension<PgPool>) -> ApiResult {
	let wizard = Wizard::get(&pool).await?;
	Ok(ApiResponse::new(json!(wizard.auto_adoption_state), StatusCode::OK))
}
