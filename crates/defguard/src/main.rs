...
async fn main() -> Result<(), anyhow::Error> {
    ...
    // run services
    tokio::select! {
        res = run_grpc_bidi_stream(pool.clone(), wireguard_tx.clone(), mail_tx.clone(), bidi_event_tx), if config.proxy_url.is_some() => error!("Proxy gRPC stream returned with error: {res:?}"),
