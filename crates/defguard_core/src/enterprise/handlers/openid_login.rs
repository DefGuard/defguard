...
#[utoipa::path(
    ...
    responses(
        ...
        (status = 400, description = "Invalid callback payload.", body = ApiErrorResponse, example = json!({"msg": "CSRF cookie not found"})),
        (status = 401, description = "CSRF token mismatch or missing nonce cookie.", body = ApiErrorResponse, example = json!({"msg": "CSRF token mismatch"})),
        (status = 403, description = "Requires an active enterprise license.", body = ApiErrorResponse, example = json!({"msg": "requires privileged access"})),
