...
/// Returns whether `method` is configured for `user` (and, for biometric, `device_id`).
///
/// This is the per-user/per-device setup state ANDed with deployment-level availability:
/// `smtp_configured` gates email and `oidc_configured` (plus a business license) gates OIDC,
/// mirroring [`compute_method_availability`]. The remaining methods have no deployment gate.
///
...
pub async fn is_method_configured<'e, E: PgExecutor<'e>>(
    ...
    let configured = match method {
        VpnClientMfaMethod::Totp => user.totp_enabled,
        VpnClientMfaMethod::Email => smtp_configured && user.email_mfa_enabled,
        VpnClientMfaMethod::Oidc => {
            is_business_license_active() && oidc_configured && user.openid_sub.is_some()
        }
        VpnClientMfaMethod::Biometric => BiometricAuth::find_by_device_id(executor, device_id)
            .await?
            .is_some(),
        VpnClientMfaMethod::MobileApprove => !BiometricAuth::find_by_user_id(executor, user.id)
            .await?
            .is_empty(),
    };
    Ok(configured)
...
mod tests {
    use defguard_common::db::{
        Id,
        models::{
            Device, DeviceType, User, biometric_auth::BiometricAuth,
            vpn_client_session::VpnClientMfaMethod,
        },
        setup_pool,
    };
    ...
    use super::is_method_configured;
    ...
    #[sqlx::test]
    async fn test_is_method_configured(_: PgPoolOptions, options: PgConnectOptions) {
        ...
        // OIDC: identity + license + provider -> configured.
        ...
        // OIDC identity present but no license -> unconfigured.
        ...
        // OIDC identity + license but no provider -> unconfigured.
        set_cached_license(Some(business_license()));
        assert!(
            !is_method_configured(
                &pool,
