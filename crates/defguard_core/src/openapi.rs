...
use super::{
    enterprise::{
        handlers::{
            self as enterprise_handlers, acl, activity_log_stream, api_tokens, enterprise_settings,
            openid_login, openid_providers,
        },
...
Errors are returned as a JSON object with a `msg` field and, for some of them, a machine-readable `code` field.",
    ...
    tags(
        ...
        (name = "LDAP", description = "LDAP connection test."),
