...
async fn test_mfa_finish_succeeds_with_totp_code(_: PgPoolOptions, options: PgConnectOptions) {
    ...
    // Verify GatewayCommand::MfaSessionAuthorized was broadcast.
    ...
    let event = timeout(RECEIVE_TIMEOUT, gateway_rx.recv())
        .await
        .expect("timed out waiting for GatewayCommand::MfaSessionAuthorized")
...
///  - emit `GatewayCommand::MfaSessionDisconnected` for the first session, and
