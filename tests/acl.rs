...
async fn test_rule_create_modify_state() {
    ...
    let response = client.put("/api/v1/acl/rule/1").json(&rule_modified).send().await;
    assert_eq!(response.status(), StatusCode::OK);
...
async fn test_rule_delete_state_applied() {
    ...
    assert_eq!(response.status(), StatusCode::OK);
    assert_eq!(AclRule::all(&pool).await.unwrap().len(), 2);
    let rule_parent: ApiAclRule<Id> =
        client.get("/api/v1/acl/rule/1").send().await.json().await;
    let rule_child: ApiAclRule<Id> =
        client.get("/api/v1/acl/rule/2").send().await.json().await;
    assert_eq!(rule_before_mods, rule_parent);
