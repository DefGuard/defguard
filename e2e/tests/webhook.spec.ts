...
test.describe('Test webhooks', () => {
  ...
  test('Create webhook and verify content', async ({ page, browser }) => {
    ...
    const webhook_description_cell = await webhookRow
      .locator('.cell-1 span')
      .textContent();
  ...
  test('Create, modify webhook and verify content', async ({ page, browser }) => {
    ...
    await page
      .locator('.edit-button-floating-ui')
    ...
    const changed_webhook_url_cell = await changed_webhookRow
      .locator('.cell-0 span')
      .textContent();
    expect(changed_webhook_url_cell).toBe(new_webhook_url);

    const changed_webhook_description_cell = await changed_webhookRow
      .locator('.cell-1 span')
      .textContent();
    expect(changed_webhook_description_cell).toBe(new_webhook_description);

    const changed_webhook_state_cell = await changed_webhookRow
      .locator('.cell-2 span')
      .textContent();
  ...
  test('Create webhook, change state and verify content', async ({ page, browser }) => {
    ...
    const changed_webhook_url_cell = await webhookRow
      .locator('.cell-0 span')
      .textContent();
    expect(changed_webhook_url_cell).toBe(webhook_url);

    const changed_webhook_description_cell = await webhookRow
      .locator('.cell-1 span')
      .textContent();
    expect(changed_webhook_description_cell).toBe(webhook_description);

    const changed_webhook_state_cell = await webhookRow
      .locator('.cell-2 span')
      .textContent();
    expect(changed_webhook_state_cell).toBe('Enabled');
