-- Create the active_wizard enum type
CREATE TYPE active_wizard AS ENUM ('none', 'initial', 'auto_adoption', 'migration');

-- Create wizard table: active_wizard and completed as columns, each wizard
...
-- Migrate initial_setup data from settings into wizard
...
-- Drop wizard-related columns from settings
ALTER TABLE settings
    DROP COLUMN initial_setup_completed,
    DROP COLUMN initial_setup_step;

-- Proxy modified_by: convert from user id (bigint FK) to user name (text)
...
-- Gateway modified_by: convert from user id (bigint FK) to user name (text)
